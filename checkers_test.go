...
func TestValidJump(t *testing.T) {
   game := New()
   src := Pos{3, 2}; dst := Pos{5, 4}; capLoc := Capture(src, dst)
...
func TestKingPiece(t *testing.T) {
   game := New()
   redDst := Pos{0, 7}; redPiece := game.Pieces[redDst]
   ...
   blkDst := Pos{1, 0}; blackPiece := game.Pieces[blkDst]
   ...
   game.Pieces[redDst] = blackPiece; game.Pieces[blkDst] = redPiece
   tmpDst := redDst; redDst = blkDst; blkDst = tmpDst
   game.kingPiece(redDst)
...
func TestMovePossibleFrom(t *testing.T) {
   ...
   blockedSrc := Pos{0, 3}; okSrc := Pos{1, 4}; okKingSrc := Pos{2, 5}
...
func TestUpdateTurnNoMove(t *testing.T) {
   ...
   blkSrc := Pos{0, 3}; redSrc := Pos{0, 5}; blockLoc := Pos{1, 4}; unblockLoc := Pos{2, 3}
...
func TestUpdateTurnNoKingJumpContinuation(t *testing.T) {
   ...
   src := Pos{4, 5}; dst := Pos{2, 7}; // dst2 := Pos{0, 5}
...
func TestUpdateTurnKingJumpContinuation(t *testing.T) {
   ...
   src := Pos{4, 5}; dst1 := Pos{2, 7}; dst2 := Pos{0, 5}
   game.Pieces[Pos{3, 6}] = Piece{RED_PLAYER, false}
   game.Pieces[Pos{1, 6}] = Piece{RED_PLAYER, false}
